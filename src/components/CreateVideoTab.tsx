...
import type { FootageRecord, MusicTrack, ParsedLesson } from '../types';
...
const MUSIC_SETTINGS_KEY = 'video-cau-truc-music-settings-v1';

interface MusicSettings {
  mode: 'none' | 'random' | 'track';
  trackId?: string;
...
function loadMusicSettings(): MusicSettings {
  try {
    ...
    if (!raw) return { mode: 'none', volumePercent: 15 };
    const parsed = JSON.parse(raw) as Partial<MusicSettings>;
    const mode = parsed.mode === 'random' || parsed.mode === 'track' ? parsed.mode : 'none';
    const volumePercent = typeof parsed.volumePercent === 'number' && Number.isFinite(parsed.volumePercent)
      ? Math.min(100, Math.max(0, Math.round(parsed.volumePercent)))
      : 15;
    return {
      mode,
    ...
    return { mode: 'none', volumePercent: 15 };
  }
}
...
export const CreateVideoTab: React.FC<CreateVideoTabProps> = ({
  ...
  useEffect(() => {
    try {
      ...
      // Settings persistence is a convenience only.
    }
  }, [musicSettings]);

  useEffect(() => {
    if (
      musicSettings.mode === 'track' &&
      musicTracks.length > 0 &&
      !musicTracks.some((track) => track.id === musicSettings.trackId)
    ) {
      setMusicSettings((current) => ({ ...current, trackId: musicTracks[0].id }));
    }
  }, [musicSettings.mode, musicSettings.trackId, musicTracks]);
  ...
  const handleStartBatch = async () => {
    ...
    if (musicSettings.mode !== 'none' && musicTracks.length === 0) {
      setScreenError('Kho nhạc máy chủ đang trống. Hãy tải nhạc lên hoặc chọn “Không dùng nhạc nền”.');
      return;
    }
    if (
      musicSettings.mode === 'track' &&
      ...
      setScreenError('Bài nhạc nền đã chọn không còn trong kho máy chủ.');
      return;
    }
    ...
    try {
      await uploadRenderBatch(
        nextBatchId,
        parsed.lessons,
        validFootages,
        {
          mode: musicSettings.mode,
          trackId: musicSettings.mode === 'track' ? musicSettings.trackId : undefined,
          volume: musicSettings.volumePercent / 100,
        },
        (progress) => {
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex min-w-0 items-start gap-3">
              <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${
                musicSettings.mode === 'none' ? 'bg-slate-800 text-slate-400' : 'bg-orange-500/10 text-orange-400'
              }`}>
                {musicSettings.mode === 'random' ? <Shuffle className="h-5 w-5" /> : <Music2 className="h-5 w-5" />}
              </div>
              <div className="min-w-0">
                <div className="text-xs font-semibold text-white">Nhạc nền cho video</div>
                <div className="mt-1 text-[11px] leading-relaxed text-slate-400">
                  Kho máy chủ hiện có {musicTracks.length} bài. Nhạc được lặp đủ thời lượng và trộn trực tiếp vào MP4.
                </div>
              </div>
            </div>
            <button
              type="button"
              onClick={onGoToMusicTab}
              className="flex w-full shrink-0 items-center justify-center gap-1 rounded-xl bg-slate-800 px-3 py-2 text-xs font-semibold text-orange-400 transition-colors hover:bg-slate-700 sm:w-auto sm:py-1.5"
            >
              Quản lý nhạc
              <ArrowRight className="h-3.5 w-3.5" />
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(180px,0.7fr)]">
            <div>
              <label className="mb-1.5 block text-[11px] font-semibold text-slate-400">Chọn nhạc</label>
              <select
                value={musicSettings.mode === 'track' ? `track:${musicSettings.trackId || ''}` : musicSettings.mode}
                disabled={activeBatch}
                onChange={(event) => {
                  const value = event.target.value;
                  if (value.startsWith('track:')) {
                    setMusicSettings((current) => ({ ...current, mode: 'track', trackId: value.slice(6) }));
                  } else {
                    setMusicSettings((current) => ({
                      ...current,
                      mode: value === 'random' ? 'random' : 'none',
                    }));
                  }
                }}
                className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2.5 text-xs text-white outline-none focus:border-orange-500 disabled:cursor-not-allowed disabled:opacity-60"
              >
                <option value="none">Không dùng nhạc nền</option>
                <option value="random" disabled={musicTracks.length === 0}>Máy chọn Random cho từng video</option>
                {musicTracks.map((track) => (
                  <option key={track.id} value={`track:${track.id}`}>{track.originalName}</option>
                ))}
              </select>
            </div>

            <div className={musicSettings.mode === 'none' ? 'opacity-50' : ''}>
              <div className="mb-1.5 flex items-center justify-between text-[11px] font-semibold text-slate-400">
                ...
                <span className="font-mono text-orange-300">{musicSettings.volumePercent}%</span>
              </div>
              <input
                id="music-volume"
                type="range"
                ...
                max="100"
                step="1"
                value={musicSettings.volumePercent}
                disabled={musicSettings.mode === 'none' || activeBatch}
                onChange={(event) => setMusicSettings((current) => ({
                  ...current,
                  volumePercent: Number.parseInt(event.target.value, 10),
                }))}
                className="h-2 w-full cursor-pointer accent-orange-500 disabled:cursor-not-allowed"
              />
              <div className="mt-1.5 text-[10px] text-slate-500">Gợi ý: 10–20% để không lấn giọng đọc.</div>
            </div>
          </div>

          {musicSettings.mode === 'random' && (
            <div className="mt-3 rounded-xl border border-purple-500/20 bg-purple-500/5 px-3 py-2 text-[11px] text-purple-200">
              Máy sẽ xáo kho nhạc và gán một bài cho từng video; chỉ lặp lại sau khi đã dùng hết các bài trong kho.
            </div>
          )}
        </div>
                  ...
                  <div key={item.id} className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        {item.musicName && (
                          <div className="mt-1 flex min-w-0 items-center gap-1 text-[10px] text-purple-300">
                            <Music2 className="h-3 w-3 shrink-0" />
                            <span className="truncate">{item.musicName}</span>
                            <span className="shrink-0 text-slate-500">· {Math.round((item.musicVolume || 0) * 100)}%</span>
                          </div>
                        )}
