...
export async function uploadRenderBatch(
  ...
  if (!Number.isFinite(music.volume) || music.volume < 0 || music.volume > 1) {
    throw new Error('Âm lượng nhạc nền phải nằm trong khoảng 0–100%.');
  }
  if (music.mode === 'track' && !music.trackId) {
    throw new Error('Chưa chọn bài nhạc nền cụ thể.');
  }
