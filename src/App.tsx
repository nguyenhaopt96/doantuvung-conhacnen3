...
import { FootageRecord, MusicTrack } from './types';
import { getAllFootage } from './lib/db';
import { fetchMusicTracks } from './lib/musicLibrary';
...
export default function App() {
  const [activeTab, setActiveTab] = useState<'create' | 'footage' | 'music' | 'history'>('create');
  ...
  const refreshMusic = async () => {
    try {
      setMusicTracks(await fetchMusicTracks());
    } catch (e) {
    ...
    >
      ...
      >
        <div className="w-full p-3 sm:p-4 md:p-6">
          ...
          <nav className="grid grid-cols-4 gap-1.5 md:block md:space-y-2">
            <button
              id="tab-create-btn"
              type="button"
            ...
            <button
              ...
              className={`relative flex min-w-0 w-full flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[10px] font-medium transition-all cursor-pointer sm:px-2 sm:text-xs md:flex-row md:justify-between md:px-4 md:py-3 md:text-base ${
                activeTab === 'music'
                  ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20 shadow-sm'
                  : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'
              }`}
            >
              <div className="flex flex-col items-center gap-1 md:flex-row md:gap-3">
                ...
                <span className="whitespace-nowrap">Kho nhạc</span>
              </div>
              <span className="hidden rounded-full bg-slate-800 px-2 py-0.5 font-mono text-xs text-slate-300 md:inline-flex">
                {musicTracks.length}
              </span>
        ...
        <div className="mt-auto hidden border-t border-slate-800/80 p-6 md:block">
          <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-4 space-y-2">
            ...
            <div className="flex justify-between items-center text-xs">
              <span className="text-slate-400">Nhạc nền:</span>
              <span className="text-orange-400 font-mono font-medium">{musicTracks.length} bài</span>
            </div>
      ...
      <main id="main-content" className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <header className="hidden h-16 shrink-0 items-center justify-between border-b border-slate-800 bg-[#0F172A]/90 px-8 backdrop-blur-md md:flex">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold text-slate-300 uppercase tracking-wider">
              ...
              {activeTab === 'music' && 'Màn hình: Kho nhạc nền (máy chủ)'}
        ...
        {activeTab === 'create' && (
          <CreateVideoTab
            ...
            onGoToMusicTab={() => setActiveTab('music')}
          />
        )}
