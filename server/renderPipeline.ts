...
import { measureAudioDuration, verifyAudioVideoMedia } from './mediaTools';
...
export interface RenderPipelineOptions {
  ...
  musicPath?: string;
  musicVolume?: number;
  ttsGenerator?: (text: string, language: 'vi' | 'en', destinationPath: string) => Promise<void>;
  onStatus?: (status: 'generating_tts' | 'rendering_video' | 'streaming') => void;
  onProgress?: (info: ProgressInfo) => void;
}

export function getAudioDuration(filePath: string): Promise<number> {
  return measureAudioDuration(filePath);
}
 ...
 * 1. File exists
 * 2. File size > 100 KB
 * 3. FFmpeg confirms that both the first video and first audio stream can be decoded.
 */
export async function verifyRenderedMp4(filePath: string): Promise<void> {
  ...
  await verifyAudioVideoMedia(filePath);
}
...
export async function runServerRenderPipeline(options: RenderPipelineOptions): Promise<{
  ...
  const { tmpDir, footagePath, lesson, musicPath, onStatus, onProgress } = options;
  const ttsGenerator = options.ttsGenerator ?? generateTTSFile;
  const musicVolume = Math.min(1, Math.max(0, options.musicVolume ?? 0));
  const hasBackgroundMusic = Boolean(musicPath && musicVolume > 0);

  if (hasBackgroundMusic && (!musicPath || !fs.existsSync(musicPath))) {
    throw new Error('Bài nhạc nền đã chọn không còn trên máy chủ.');
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await ttsGenerator(pair.vi, 'vi', viPath);
    await ttsGenerator(pair.en, 'en', enPath);
  ...
  // Measure audio durations with the same FFmpeg binary used for rendering.
  ...
  // Optional input 14: loop the selected server-side music for the full video.
  ...
  if (hasBackgroundMusic) {
    audioFilters.push(
      `[a_vi0][a_tk0][a_en0][a_vi1][a_tk1][a_en1][a_vi2][a_tk2][a_en2]amix=inputs=9:dropout_transition=0:normalize=0[voice]`,
      `[14:a]atrim=duration=${totalDuration.toFixed(3)},asetpts=PTS-STARTPTS,aresample=48000,volume=${musicVolume.toFixed(3)}[bgm]`,
      `[voice][bgm]amix=inputs=2:duration=first:dropout_transition=0:normalize=0,alimiter=limit=0.95[aout]`
    );
  } else {
    audioFilters.push(
  ...
  // 6. Verification: exit code 0, file > 100 KB, decodable video + audio
