...
import { getMusicTrackForRender, listMusicTracksForRender, type MusicTrackForRender } from './musicLibrary';
...
// PandaStack mounts /app read-only at runtime. Jobs therefore live in the
// writable runtime area and remain available while the current instance lives.
const BATCH_ROOT = runtimeStoragePath('video_cau_truc_batches');
...
function parseMusicConfig(value: unknown): BatchMusicConfig {
  if (value === undefined || value === null) return { mode: 'none', volume: 0 };
  if (!value || typeof value !== 'object') throw new Error('Cấu hình nhạc nền không hợp lệ.');

  const source = value as Record<string, unknown>;
  const mode = source.mode;
  if (mode !== 'none' && mode !== 'random' && mode !== 'track') {
    throw new Error('Chế độ chọn nhạc nền không hợp lệ.');
  }

  const volume = source.volume === undefined ? 0.15 : source.volume;
  if (typeof volume !== 'number' || !Number.isFinite(volume) || volume < 0 || volume > 1) {
    throw new Error('Âm lượng nhạc nền phải nằm trong khoảng 0–100%.');
  }

  if (mode === 'track') {
    const trackId = typeof source.trackId === 'string' ? source.trackId : '';
    if (!trackId) throw new Error('Chưa chọn bài nhạc nền cụ thể.');
    return { mode, trackId, volume };
  }
  return { mode, volume: mode === 'none' ? 0 : volume };
}

function parseManifest(rawManifest: string | undefined): ParsedBatchManifest {
  ...
  const source = parsed as { lessons?: unknown; music?: unknown };
  ...
  const items = source.lessons.map((entry, index) => {
  ...
  return {
    items,
    music: parseMusicConfig(source.music),
  };
}

function shuffledTracks(values: MusicTrackForRender[]): MusicTrackForRender[] {
  const result = [...values];
...
function prepareMusicAssignments(
  config: BatchMusicConfig,
  itemCount: number,
  batchDir: string
): Array<MusicAssignment | undefined> {
  if (config.mode === 'none') return Array.from({ length: itemCount }, () => undefined);

  let selected: MusicTrackForRender[];
  if (config.mode === 'track') {
    const track = config.trackId ? getMusicTrackForRender(config.trackId) : null;
    if (!track) throw new Error('Bài nhạc nền đã chọn không còn trong kho máy chủ.');
    selected = Array.from({ length: itemCount }, () => track);
  } else {
    const available = shuffledTracks(listMusicTracksForRender());
    if (available.length === 0) throw new Error('Kho nhạc máy chủ đang trống, không thể chọn ngẫu nhiên.');
    selected = Array.from({ length: itemCount }, (_, index) => available[index % available.length]);
  }

  const musicDir = path.join(batchDir, 'music');
  fs.mkdirSync(musicDir, { recursive: true });
  const copiedPaths = new Map<string, string>();

  return selected.map((track) => {
    let destination = copiedPaths.get(track.id);
    if (!destination) {
      const extension = /^\.[a-z0-9]{1,8}$/i.test(path.extname(track.filePath))
        ? path.extname(track.filePath).toLowerCase()
        : '.audio';
      destination = path.join(musicDir, `${track.id}${extension}`);
      fs.copyFileSync(track.filePath, destination);
      copiedPaths.set(track.id, destination);
    }
    return {
      trackId: track.id,
      musicName: track.originalName,
      musicPath: destination,
  ...
  app.post('/api/render-batches', (req: Request, res: Response) => {
    ...
    parser.on('close', async () => {
      try {
        ...
        const manifest = parsedManifest.items;
        ...
        const musicAssignments = prepareMusicAssignments(parsedManifest.music, manifest.length, batch.tmpDir);
        ...
        batch.items = manifest.map((manifestItem, index) => ({
          ...
          musicTrackId: musicAssignments[index]?.trackId,
          musicName: musicAssignments[index]?.musicName,
          musicPath: musicAssignments[index]?.musicPath,
          musicVolume: musicAssignments[index]?.musicVolume,
          status: 'queued',
          progress: 0,
          currentTimeSec: 0,
          totalDuration: 0,
